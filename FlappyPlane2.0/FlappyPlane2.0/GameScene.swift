...
protocol GameSceneDelegate: AnyObject {
    func gameSceneDidFinish(_ gameScene: GameScene)
}
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    @objc func updateTime() {
        timerLabel.text = "Time: \(String(format: "%.1f", timeElapsed))"
        timeElapsed += 0.1
    }
    ...
    func stupTimerLabel() {
        timerLabel = SKLabelNode(fontNamed: "MuktaMahee-Medium")
        timerLabel.fontColor = .red
        timerLabel.fontSize = 24
        timerLabel.position = CGPoint(x: size.width - 80, y: size.height - 150)
        addChild(timerLabel)
    }
    func setupGameOverLabel() {
        gameOverLabel = SKLabelNode(fontNamed: "MuktaMahee-Medium")
        gameOverLabel.text = "GAME OVER \nTime in the air : \(String(format: "%.1f", timeElapsed - 0.1))"
          ...
          for block in blocks {
              if airplane.intersects(block) {
                  ...
                  timer?.invalidate()
                      timer = nil
                  setupGameOverLabel()
                  let savedProgress = ModelForSavingProgress(timeInTheAir: "\(timeElapsed)")
                  RealmManager.shared.saveModel(model: savedProgress)
                  if let realmURL = getRealmDatabaseURL() {
                      print("Realm database URL: \(realmURL)")
                  }
                  return
              }
          }
    ...
    func setupButtons() {
        let menuButtonSize = CGSize(width: 130, height: 50)
        menuButton = UIButton(type: .system)
        menuButton.frame = CGRect(x: 20, y: 50, width: menuButtonSize.width, height: menuButtonSize.height)
        let menuButtonAttributedTitle = NSAttributedString(string: "MENU",
                                                         attributes: [NSAttributedString.Key.font : UIFont(name: "MuktaMahee-Medium", size: 23)!,
                                                                      NSAttributedString.Key.foregroundColor : UIColor.white])
        menuButton.setAttributedTitle(menuButtonAttributedTitle, for: .normal)
        menuButton.addGradient(colors: [UIColor.purple, UIColor.black], startPoint: CGPoint(x: 0.0, y: 0.0), endPoint: CGPoint(x: 1.0, y: 1.0), transform: nil)
